...
#[uniffi::export]
pub fn initialize(whisper_path: String, llm_path: String) -> Result<(), String> {
    let mut state = STATE.lock().map_err(|e| e.to_string())?;
    state.whisper_model_path = Some(whisper_path);
    state.llm_model_path = Some(llm_path);
    
    // 初始化 SQLite 資料庫
    database::init_db().map_err(|e| e.to_string())?;
    Ok(())
}
...
#[uniffi::export]
pub fn stop_recording_and_process(style: String) -> Result<String, String> {
    let mut state = STATE.lock().map_err(|e| e.to_string())?;
    if !state.is_recording {
    ...
    let llm_model = state.llm_model_path.as_ref()
        .ok_or_else(|| "LLM model not initialized".to_string())?;
    ...
    // 5. 存入本地歷史紀錄
    database::save_history(&formatted_text).map_err(|e| e.to_string())?;
