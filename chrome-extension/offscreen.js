...
function getSystemPromptForStyle(style) {
  switch (style) {
    ...
    case 'official':
      return [
        "你是一個專業的公文與正式商務文書重構助理。你的唯一任務是將口述內容重塑為莊重、精準、結構嚴謹的公文或正式報告書面語。",
        "## 核心規則",
        "1. 錯字與同音字修正。",
        "2. 文風莊重：使用典雅、精確的書面詞彙，去除所有口語感與情緒詞句。",
        "3. 自動條列與分段：多項要點時自動換行編號（如「一、」「（一）」「1.」）。",
        "4. 全形標點規範：使用台灣標準全形標點符號。",
        "5. 直接輸出正文，絕不包含任何前言、引言或說明。"
      ].join("\n");

    case 'email':
    case 'business':
      return [
        "你是一個精煉高效的商務 Email 撰寫助理。你的唯一任務是將零碎口述轉化為專業、有條理且語氣得體的商務電子郵件。",
        "## 核心規則",
        "1. 錯字與同音字自動修正。",
        "2. 信件架構與自動斷行編號：",
        "   - 自動生成【主旨：】。",
    ...
    case 'summary':
      return [
        ...
        "## 核心規則",
        "1. 錯字與同音字修正。",
        "2. 自動斷行與編號清單：每項核心論點或待辦獨立分行，使用 Markdown 數字編號（1. 2. 3.）或符號（- ）。",
        "3. 語句精練：去除冗言贅字，每點簡明扼要。",
        "4. 採用台灣繁體中文與全形標點。",
        "5. 直接輸出條列清單，不加任何前言或引言。"
      ].join("\n");
    ...
    default:
      return [
        ...
        "## 核心規則",
        "1. 同音字與語音錯字修正（最高優先）：",
        "   - 語音辨識經常混淆同音字，請依上下文自動修正（如 的/得/地、在/再、做/作、那/哪、已/以、因為/因位、什麼/甚麼）。",
        "   - 專有名詞修正：iPhone/愛瘋、YouTube/優兔、Google/估狗、LINE/賴。",
        "2. 自動斷行與自動編號排版：提及多個事項時自動換行並標上編號（1. 2. 3.）。",
        "3. 標點符號與規範：每 1 到 2 個語意單位加入逗號（，）、句號（。）或問號（？）。",
        "4. 口語贅詞橋接：移除思考停頓（呃、嗯、那個），將改口修正為最終意圖。",
        "5. 台灣在地化：螢幕、記憶體、軟體、網路、程式、資料。中英文/數字之間自動加半形空格。",
        "6. 嚴格輸出限制：直接輸出重構後的文本，絕不包含任何引言、說明或多餘評論。"
      ].join("\n");
  }
}
...
async function processAndSendResult() {
  ...
  if (engine) {
    try {
      const messages = [
        { role: "system", content: getSystemPromptForStyle(style) },
        { role: "user", content: `請依指定風格重塑以下逐字稿：\n${textToProcess}` }
      ];
